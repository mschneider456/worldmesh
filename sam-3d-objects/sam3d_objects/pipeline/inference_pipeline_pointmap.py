...
class InferencePipelinePointMap(InferencePipeline):
    ...
    def compute_pointmap(self, image, pointmap=None, fov_x=None):
        ...
        if pointmap is None:
            with torch.no_grad():
                with torch.autocast(device_type="cuda", dtype=self.dtype):
                    output = self.depth_model(loaded_image, fov_x=fov_x)
        ...
        if intrinsics is None:
            ...
            intrinsics_result = infer_intrinsics_from_pointmap(
                points_tensor_moge, fov_x=fov_x, device=self.device
            )
    ...
    def run(
        ...
        image = self.merge_image_and_mask(image, mask)
        with self.device:
            pointmap_dict = self.compute_pointmap(image, pointmap, fov_x=fov_x)
            pointmap = pointmap_dict["pointmap"]
