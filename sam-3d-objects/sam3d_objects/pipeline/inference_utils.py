...
def downsample_sparse_structure(
    ...
    Args:
        ...
        max_coords: maximum number of coordinates to keep
            30000 is a practical GPU memory limit. The theoretical int32 limit is ~43691
            (max(int32) / (64*768)) but objects above ~40K coords OOM during mesh
            decoding in practice. Only needed for mesh decoding.
    ...
    """
    original_count = coord_batch.shape[0]
    print(f"    Sparse structure: {original_count} coords (max: {max_coords})")

    if original_count <= max_coords:
    ...
    if unique_combined.shape[0] > max_coords:
        print(f"    After spatial downsampling: {unique_combined.shape[0]} coords, random subsampling to {max_coords}")
        indices = torch.randperm(unique_combined.shape[0], device=coord_batch.device)[
            :max_coords
        ]
        unique_combined = unique_combined[indices]
    else:
        print(f"    After spatial downsampling: {unique_combined.shape[0]} coords")
