...
descriptions = {
    ...
    "depth-splatfacto": "Gaussian Splatting with depth supervision for improved geometry.",
}
    ...
    pipeline=VanillaPipelineConfig(
        datamanager=FullImageDatamanagerConfig(
            ...
            dataparser=ColmapDataParserConfig(
                load_3D_points=True,
                depths_path=Path("depths"),  # Enable depth loading from depths/ folder
                depth_unit_scale_factor=1e-3,  # Convert from mm (16-bit PNG) to meters
            ),
            cache_images_type="uint8",
        ),
